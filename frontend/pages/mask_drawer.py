...
# bg_image_url = image_to_data_url(image)
...
#     stroke_width=5,
#     stroke_color="#FF0000",
#     background_image=image.convert("RGB"),  # Pass PIL Image directly here
...
image_files = sorted(glob.glob("frontend/images/*.png"))
if not image_files:
    st.error("No images found in 'images/' folder.")
    st.stop()
...
selected_image_path = os.path.join("frontend/images", selected_image_name)
selected_mask_path = os.path.join("frontend/masks", selected_image_name)

if not os.path.exists(selected_image_path) or not os.path.exists(selected_mask_path):
    st.error(f"Image or mask not found for: {selected_image_name}")
    st.stop()
...
# Convert image to base64 and inject CSS
bg_base64 = image_to_base64(image)
canvas_style = f"""
    <style>
        .canvas-wrap {{
            position: relative;
...
# Canvas (transparent, over background)
canvas_result = st_canvas(
    fill_color="rgba(255, 0, 0, 0.4)",
    stroke_width=5,
    stroke_color="#FF0000",
    background_image=None,  # Omit this
    update_streamlit=True,
    height=512,
    width=512,
    drawing_mode="freedraw",
...
def overlay_mask_on_image(image_pil, mask_np, mask_color=(255, 0, 0, 100)):
    ...
    color_mask = Image.new("RGBA", image_pil.size, mask_color)
    color_mask.putalpha(mask_img)
    return Image.alpha_composite(image_rgba, color_mask)
...
if st.button("Submit Mask"):
    if canvas_result.image_data is not None:
        alpha_channel = canvas_result.image_data[:, :, 3]
        if np.any(alpha_channel > 0):
            user_mask = (alpha_channel > 0).astype(np.uint8)
            mask_gt_np = np.array(mask_gt)
            mask_gt_bin = (mask_gt_np > 127).astype(np.uint8)
